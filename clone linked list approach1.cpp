...
class Solution
{
    ...
    Node *copyList(Node *head)
    {
       ...
       while(temp!=NULL){
           insertAtTail(cloneHd, cloneTl, temp->data);
           temp = temp -> next;
       }
       
       //create a map for mapping the cloneNode with the originalNode
       ...
       while(temp!=NULL && temp2!=NULL){
           Node* ogNode = head;
           Node *clnNode = cloneHd;
           while(ogNode!=NULL && clnNode!=NULL){
               oldToNewNode[ogNode]= clnNode;
               ogNode = ogNode -> next;
           ...
           while(ogNode!=NULL){
               clnNode -> arb = oldToNewNode[ogNode -> arb];
               ogNode = ogNode->next;
               clnNode = clnNode-> next;
           }
